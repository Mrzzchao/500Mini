...
Page({
  ...
  getHistory() {
    app.utils.Ajax.getHotHistory(4).then((data) => {
      this.setData({ historyList: data });
    })
  },

  getOpenInfo() {
    app.utils.Ajax.getHotInfo(4).then((data) => {
      if (data.curr_expect) {
  ...
  getPrizeDetail() {
    app.utils.Ajax.getHotPrizeDetail({ lotid: 4, expect: this.data.expect }).then((data) => {
      if (data && data.OutBall) {
        data['fore'] = data.OutBall.split(',');
      }
