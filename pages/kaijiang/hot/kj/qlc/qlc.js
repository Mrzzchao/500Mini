...
Page({
  ...
  getHistory() {
    app.utils.Ajax.getHotHistory(11).then((data) => {
      this.setData({ historyList: data });
    })
  },

  getOpenInfo() {
    app.utils.Ajax.getHotInfo(11).then((data) => {
      if (data.curr_expect) {
  ...
  getPrizeDetail() {
    app.utils.Ajax.getHotPrizeDetail({ lotid: 11, expect: this.data.expect }).then((data) => {
      if (data && data.result) {
