...
Page({
  ...
  getHistory() {
    app.utils.Ajax.getHotHistory(17).then((data) => {
      this.setData({ historyList: data });
    })
  },

  getOpenInfo() {
    app.utils.Ajax.getHotInfo(17).then((data) => {
      if (data.curr_expect) {
  ...
  getPrizeDetail() {
    app.utils.Ajax.getHotPrizeDetail({ lotid: 17, expect: this.data.expect }).then((data) => {
      if (data && data.result) {
