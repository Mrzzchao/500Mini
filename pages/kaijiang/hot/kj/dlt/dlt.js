...
Page({
  ...
  getHistory() {
    app.utils.Ajax.getHotHistory(28).then((data) => {
      this.setData({ historyList: data });
    })
  },

  getOpenInfo() {
    app.utils.Ajax.getHotInfo(28).then((data) => {
      if (data.curr_expect) {
  ...
  getPrizeDetail() {
    app.utils.Ajax.getHotPrizeDetail({ lotid: 28, expect: this.data.expect }).then((data) => {
      if (data && data.UnsortOutBall) {
        var str = data.UnsortOutBall.split('|');
        data['fore'] = str[0].split(',').sort();
        data['back'] = str[1].split(',').sort();
      }
