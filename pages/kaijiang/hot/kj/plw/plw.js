...
Page({
  ...
  getHistory() {
    app.utils.Ajax.getHotHistory(10001).then((data) => {
      this.setData({ historyList: data });
    })
  },

  getOpenInfo() {
    app.utils.Ajax.getHotInfo(10001).then((data) => {
      if (data.curr_expect) {
  ...
  getPrizeDetail() {
    app.utils.Ajax.getHotPrizeDetail({ lotid: 10001, expect: this.data.expect }).then((data) => {
      if (data && data.result) {
